...
namespace MySql.Web.Security
{
    ...
    /// and managing passwords for MySql database.
    /// </summary>
    /// <remarks>MySql.Web.Security has MySqlMembershipProvider derived from MembershipProvider, but this is not good enough for MVC apps.</remarks>
    public class MySqlSimpleMembershipProvider : ExtendedMembershipProvider
    {
        ...
        {
            Trace.TraceInformation("At service startup, MySqlSimpleMembershipProvider created.");
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="MySqlSimpleMembershipProvider"/> class.
        /// Used by WebSecurity in MVC
        /// </summary>
        ...
        public MySqlSimpleMembershipProvider(MembershipProvider previousProvider)
        {
            ...
            {
                throw new ArgumentNullException("previousProvider", "prevousProvider must not be null.");
            }

            Trace.TraceInformation("At service startup, MySqlSimpleMembershipProvider created with previousProvider.");
        ...
        private MySqlSecurityDbContext NewMySqlSecurityDbContext
        {
            get
            {
                ...
                //else
                {
                    ...
                    try
                    {
                        Type contextType = Type.GetType(ConfigUtil.MySqlSecurityInheritedContextType, true, true);
                    ...
                    {
                        Trace.TraceWarning("When creating instances of {0}, there is error: {1}", ConfigUtil.MySqlSecurityInheritedContextType, ex.ToString());
                        throw;
                    }
                }
            }
        ...
        /// <returns>The user ID. -1 if not found</returns>
        ...
        {//todo: replace IDictionary<string, object> values with an interface to the derived class of UserProfile, or just get rid of CreateUserRow.
            ...
            if (values != null && values.Count > 0)
            {
                ...
                var contextAssembly = Assembly.GetAssembly(type);//expect a derived class of UserProfile with more fields
        ...
        private static bool SetPassword(MySqlSecurityDbContext db, int userId, string newPassword)
        {
            ...
            membership.PasswordChangedDate = DateTime.UtcNow;//fixed from Now. Z
        ...
        public override string GeneratePasswordResetToken(string userName, int tokenExpirationInMinutesFromNow)
        {
            ...
            using (var db = NewMySqlSecurityDbContext)
            {
                ...
                //proven in TestDateTimeNullable, comparison with a null nullable date is always false.
                var membership = db.Memberships.SingleOrDefault(x => x.UserId == userId
                    && ((x.PasswordVerificationTokenExpirationDate == null) || (x.PasswordVerificationTokenExpirationDate > DateTime.Now)));
        ...
        public override bool ResetPasswordWithToken(string token, string newPassword)
        {
            ...
            using (var db = NewMySqlSecurityDbContext)
            {
                var membership = db.Memberships.SingleOrDefault(x => x.PasswordVerificationToken == token
                    && ((x.PasswordVerificationTokenExpirationDate == null) || (x.PasswordVerificationTokenExpirationDate > DateTime.Now)));
        ...
        internal void ValidateTable()
        {
            using (var db = NewMySqlSecurityDbContext)
            {
                ...
                {
                    (new System.Data.Entity.DropCreateDatabaseAlways<MySqlSecurityDbContext>()).InitializeDatabase(db);
                }
